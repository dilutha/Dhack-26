  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!mounted) return;
        const list = (json?.data || []) as HackEvent[];
        setEvents(list);
      } catch {}
    };
    load();
                  ...
                  return (
                    ...
                    >
                      ...
                      {isCurrentMonth && hasEvent && (
                        <div className='absolute bottom-0 sm:bottom-1 flex gap-0.5 sm:gap-0.5'>
                          {day.events.slice(0, 4).map((ev, i) => (
                            <span
                              key={i}
                              className={`w-2 h-2 sm:w-1.5 sm:h-1.5 rounded-full ${getTypeBgColor(ev.type)} shadow-sm`}
                            />
                          ))}
                        </div>
                      )}
